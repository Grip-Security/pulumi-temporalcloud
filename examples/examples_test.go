...
func getGoBaseOptions(t *testing.T) integration.ProgramTestOptions {
	...
	baseJS := base.With(integration.ProgramTestOptions{
		Dependencies: []string{
			fmt.Sprintf("github.com/Grip-Security/pulumi-temporalcloud/sdk=%s", rootSdkPath),
		},
		Env: []string{
...
func getCSBaseOptions(t *testing.T) integration.ProgramTestOptions {
	...
	baseJS := base.With(integration.ProgramTestOptions{
		Dependencies: []string{
			"GripSecurity.PulumiTemporalCloud",
		},
	})
...
func checkCreds(t *testing.T) {
	var exists bool
	_, exists = os.LookupEnv("TEMPORAL_CLOUD_API_KEY")
	if !exists {
		t.Skipf("Skipping test due to missing TEMPORAL_CLOUD_API_KEY environment variable")
	}
}
